      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          if (!process.env.GOOGLE_API_KEY) {
            throw new functions.https.HttpsError("internal", "Server configuration error: GOOGLE_API_KEY missing.");
          }
...
If the car is unclear, leave values empty. Do not include any explanation. If you don't know the exact year, provide a possible year instead of a range of years.`,
            },
          ];
          ...
          if (isOverloaded && attempt < MAX_RETRIES) {
            ...
            throw new functions.https.HttpsError("internal", "Gemini API failed after 3 attempts.");
          } else {
            throw new functions.https.HttpsError("internal", "Error analyzing image.");
          }
        }
      }

      throw new functions.https.HttpsError("internal", "Failed to analyze image after retries.");
    });
  });
